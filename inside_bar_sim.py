...
def get_trades_df(df_raw):
    ...
    #df['ask_high_prev] = df.ask_h.shift(1)
    #df['bid_low_prev] = df.bid_low.shift(1)
    ...
    df_trades['trade_end'] = df_trades.next + pd.to_timedelta(55, unit="s")
    df_trades['trade_start'] = df_trades.open_time + pd.to_timedelta(1, unit="min")
...
def evaluate_pair(df_trades, s5_data):
    total = 0
    for index, row in df_trades.iterrows():
        s5_slice = s5_data[(s5_data.open_time >= row.trade_start) & (s5_data.open_time <= row.trade_end)]
        #s5_data = df_raw[(df_raw.open_time >= row.trade_start) & (df_raw.open_time <= row.trade_end)]
        ask_close = (s5_slice.close.values * 1.002)
        bid_close = (s5_slice.close.values * 0.998)
        if row.SIGNAL == BUY:
            r = process_buy(row.TAKEPROFIT, row.STOPLOSS, ask_close, bid_close, row.ENTRY)
            total += r
        else:
            r = process_sell(row.TAKEPROFIT, row.STOPLOSS, ask_close, bid_close, row.ENTRY)
            total += r
    return total
...
def run():
    ...
    for pairname in test_pairs:
        ...
        s5_data = pd.read_csv(utils.get_his_data_filename(pairname, granularity))
        s5_data["open_time"] = pd.to_datetime(s5_data["open_time"], errors="coerce")
        #price_data = processs_data(ma_short, ma_long, price_data)

        df_trades = get_trades_df(s5_data)
        score = evaluate_pair(df_trades, s5_data)
        grand_total += score
