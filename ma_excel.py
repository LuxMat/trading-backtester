...
def add_pair_charts(ma_test_res, all_trades, writer=None):
    ...
    for index, row in df_temp.iterrows():
        ...
        temp_all_trades['CUM_GAIN'] = temp_all_trades.GAINS.cumsum()
...
def create_excel(ma_test_res, all_trades):
    ...
    #Time: 21:00:00+22:22, where 22:22 is "timezone awareness", excel don't accept,
    #timezone aware date.
    #all_trades['open_time'] = [x.replace(tzinfo=None) for x in all_trades.open_time]
    all_trades['CROSS'] = "MA_" + all_trades['MASHORT'].map(str) + "_" + all_trades['MALONG'].map(str)
    all_trades["open_time"] = pd.to_datetime(all_trades["open_time"], utc=True).dt.tz_convert(None)
